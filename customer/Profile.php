...
$selectRes = $connection -> prepare($selectUser);
...
if(isset($_POST['btnUpdateUser'])){
    ...
    $checkUserEmail = "SELECT COUNT(*) FROM users WHERE Email=? AND UserID != ?"; // keep the same email
    $checkRes = $connection -> prepare($checkUserEmail);
    ...
    if(!$validation['nameStatus'] && !$validation['emailStatus'] && !$validation['phoneStatus'] && !$validation['addressStatus'] && !$validation['duplicateEmailStatus']){
        ...
        {
            $updateUser = "UPDATE users SET FullName=?, Email=?, Phone=?, Address=? WHERE UserID=?";
            $updateRes = $connection -> prepare($updateUser);
            ...
            $updateUser = "UPDATE users SET FullName=?, Email=?, Phone=?, Address=?, ProfileImage=? WHERE UserID=?";
            $updateRes = $connection -> prepare($updateUser);
